...
比如32位JVM的`Mark Word`的默认存储结构如下表：
...
在Java SE1.6中，锁一共有4种状态，**级别从低到高**依次是：
...
当一个线程访问同步块并获取锁时，会在**对象头**和**栈帧中的锁记录**里**存储偏向的线程ID**，以后该线程在进入和退出同步块时不需要进行CAS操作来加锁和解锁，只要简单地测试一下对象头的Mark Word里是否存储着指向当前线程的偏向锁：
...
偏向锁的撤销，需要等待全局安全点，也就是某一个时间点上没有正在执行的字节码。
...
但如果存在锁竞争，除了互斥量的本身的开销外，还额外发生了CAS操作的开销。
...
当有新的线程请求锁时，可以在不放弃处理器执行时间的同时，让线程通过执行一个忙循环（自旋）进行等待，以观察是否当前持有锁的线程很快释放锁。
